...
export default function Home() {
  return (
    <>
      {/* Hero */}
      <section className="relative min-h-screen flex items-center bg-purple-800 overflow-hidden">
        {/* Background photo */}
        <Image
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 md:py-40">
          <div className="max-w-3xl">
            ...
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
              Value Your{" "}
              <span className="text-teal-400">Business</span> Today
            </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {stats.map((s) => (
              <div key={s.label} className="text-center py-6 px-4 rounded-xl border border-gray-100 hover:shadow-md transition-shadow">
                <p className="font-heading text-4xl font-bold text-purple-700 mb-1">{s.value}</p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-14 items-center">
            <div>
              <p className="text-teal-500 font-semibold text-sm tracking-widest uppercase mb-3">Who We Are</p>
            ...
            <div className="bg-purple-800 rounded-2xl p-8 text-white">
              <h3 className="font-heading text-xl font-semibold mb-6 text-teal-400">Working Experience With</h3>
              <div className="grid grid-cols-2 gap-4">
                {["KPMG", "PwC", "Fatima Group", "Starlet"].map((co) => (
                  <div key={co} className="bg-purple-700 rounded-lg px-4 py-3 text-center font-semibold text-sm">
                    {co}
                  </div>
                ))}
              </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-teal-500 font-semibold text-sm tracking-widest uppercase mb-3">What We Do</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            {servicePreview.map((s) => (
              <Link key={s.title} href={s.href}
                className="group bg-gray-50 hover:bg-purple-800 border border-gray-100 hover:border-purple-800 rounded-xl p-6 transition-all duration-300">
                <p className="text-2xl mb-3">{s.icon}</p>
                <p className="font-heading font-semibold text-purple-800 group-hover:text-white transition-colors">{s.title}</p>
              </Link>
            ))}
          </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-teal-500 font-semibold text-sm tracking-widest uppercase mb-3">Trusted By</p>
            <h2 className="section-heading">Our Clients</h2>
          </div>
          <div className="flex flex-wrap justify-center gap-4">
            {clients.map((c) => (
              <span key={c} className="bg-white border border-gray-200 text-gray-600 font-medium text-sm px-5 py-2.5 rounded-full shadow-sm hover:border-teal-400 hover:text-purple-800 transition-colors">
                {c}
              </span>
            ))}
          </div>
